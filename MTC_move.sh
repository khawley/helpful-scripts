...
echo "~~Making list of all files in directory"
#echo -n "  ~~what file should I save the directory list to? "
#read directory
directory="directory"
cd ~/Desktop/MTC_new
...
do
	if [ -d '$X' ]
	then
		echo " ^^ $X is directory" >>log
	else
		if [ "$X" == "campership.html" ]
		then
			echo " ^^^ $X skipped"
		else
			echo $X >> $directory
...
echo "~~Filtering directories out and creating formated input by prepending '/'"
echo "------------------- add_root -------------------" >> log
add_root $directory "/"

echo "~~Running line_rename.sh for directory adjustment"
direc_adj=$directory'_format'
...
rm .sedscript_temp
mv log ../log
mv $direc_adj ../$direc_adj
rm $directory #$direc_adj
